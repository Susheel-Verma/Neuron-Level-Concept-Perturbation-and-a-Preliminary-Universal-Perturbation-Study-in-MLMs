...
class trainer:
    ...
    def find_mm_neurons(self, eval_data):
        ...
        for data in tqdm(test_dataloader):
            ...
            if self.args.model_type == 'llava':
                image_tensor = self.image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
                img_tensor = self.image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            ...
            # safe fallback
            if 'new_nouns' not in locals():
                new_nouns = []

            if 'new_nouns' in locals():
                nouns = new_nouns
            else:
                nouns = []
            ...
            if 'noun_pos' in locals() and len(noun_pos) > 0:
                torch.save(last_activation[noun_pos].cpu(), f'{self.args.save_to}/{cnt}/last_activation')
            else:
                torch.save(last_activation.cpu(), f'{self.args.save_to}/{cnt}/last_activation')
            ...
            for j, (token, noun, pos) in enumerate(zip(noun_tokens, nouns, noun_pos)):
                ...
                try:
                   ...
                   print("⚠️ Skipping invalid image shape:", img.shape)
                   continue
                ...
                if self.args.model_type == 'llava':
                    ...
                    for mean_num in [1, 10, 50, 100, 500, 1000]:
                        ...
                        mean_activation = torch.stack(
                            [activations[x[0], :, x[1]].reshape(16, 16, 1) for x in mm_neurons[:mean_num]]
                        ).mean(dim=0).numpy()

                        new_activation = bilinear_interpolation(
                            mean_activation,
                            img_tensor.shape[1],   # width
                            img_tensor.shape[0]    # height
                        )

                        # ---------------- HEATMAP ----------------
                        new_activation_ = (new_activation - new_activation.min()) / (
                            new_activation.max() - new_activation.min() + 1e-8
                        )
                        ...
                        if image_rgba.size != heatmap.size:
                            heatmap = heatmap.resize(image_rgba.size)
                        new_image = Image.alpha_composite(image_rgba, heatmap).convert('RGB')
                        ...
                        if image_rgba.size != mask.size:
                            mask = mask.resize(image_rgba.size)
                        new_image = Image.alpha_composite(image_rgba, mask).convert('RGB')
                    ...
                    for i in range(5):
                        ...
                        new_activation = bilinear_interpolation(activation, img_tensor.shape[1], img_tensor.shape[0])
                        ...
                        for i_, j_ in zip(pos[0], pos[1]):
                            if 0 <= j_ <width and 0 <= i_ <height:
                                mask_pixels[j_, i_] = (255, 255, 255, 0)
        ...
        save_dir = self.args.save_to
        os.makedirs(save_dir, exist_ok=True)
        
        if hasattr(self, "activations"):
            torch.save(
                self.activations,
                os.path.join(self.args.save_to, "activations.pt")
            )
            print("Activations saved")
        else:
            print("No global activations found, skipping save")
